...
function rectangleRectangle(r1, r2) {
	return overlap(r1.min.x, r1.max.x, r2.min.x, r2.max.x) && overlap(r1.min.y, r1.max.y, r2.min.y, r2.max.y);
}
...
function rayRectangle(r, b) {
	...
	var T_xmin = (b.min.x - r.start.x)/ray_dx;
	var T_xmax = (b.max.x - r.start.x)/ray_dx;
	var T_ymin = (b.min.y - r.start.y)/ray_dy;
	var T_ymax = (b.max.y - r.start.y)/ray_dy;
	
	if (T_ymax < T_xmax || T_xmin < T_ymin) {
		return null;
	}
	
	var T = Math.max(T_xmax, T_ymin);
	var T2 = Math.min(T_xmin, T_ymax);
